...
// Generate shadow_ext_001 to shadow_ext_100 with guaranteed unique sig parameters
for (let i = 1; i <= 100; i++) {
  const pad = i.toString().padStart(3, '0');
  const baseImg = unsplashImages[(i - 1) % unsplashImages.length];
  const uniqueImg = `${baseImg}&sig=shadow_${pad}`;
  mappingEntries.push(`  "shadow_ext_${pad}": "${uniqueImg}",`);
}

// Generate listen_001 to listen_101 with guaranteed unique sig parameters
for (let i = 1; i <= 101; i++) {
  const pad = i.toString().padStart(3, '0');
  const baseImg = unsplashImages[(i + 13) % unsplashImages.length];
  const uniqueImg = `${baseImg}&sig=listen_${pad}`;
  mappingEntries.push(`  "listen_${pad}": "${uniqueImg}",`);
}

const fileContent = `// High-resolution Topic & Explicit Lesson Cover Image Mapper for ALL 200+ XP English Lessons
...
fs.writeFileSync(path.join(__dirname, '../lib/utils/lessonImageMapper.ts'), fileContent, 'utf-8');
console.log('Successfully wrote unique lessonImageMapper.ts!');
