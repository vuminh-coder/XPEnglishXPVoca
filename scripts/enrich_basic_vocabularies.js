...
// Load existing basicVocabularies
const existingBasicPath = path.join(__dirname, '../lib/data/basicVocabularies.ts');
const fileStr = fs.readFileSync(existingBasicPath, 'utf8');

// We extract themes and existing vocabularies
const themesMatch = fileStr.match(/export const BASIC_VOCABULARY_THEMES: BasicTheme\[\] = (\[[\s\S]*?\]);\n\n\/\*\*/);
if (!themesMatch) {
  console.error("Failed to parse BASIC_VOCABULARY_THEMES");
  process.exit(1);
}
...
console.log("THEMES defined successfully: " + THEMES.length);
