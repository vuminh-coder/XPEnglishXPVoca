...
export const GroupCardItem: React.FC<GroupCardItemProps> = ({ group, onJoinToggle }) => {
  const icon = GROUP_ICONS[group.id] || <Users className="w-5 h-5 text-indigo-500" />;

  return (
    <div className="p-4 sm:p-5 rounded-xl bg-slate-50 dark:bg-slate-800/50 border border-slate-200/70 dark:border-slate-800 hover:border-slate-300 dark:hover:border-slate-700 flex flex-col justify-between space-y-3.5 transition-all shadow-2xs">
