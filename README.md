...
- **`/dashboard`**: Trung tâm chỉ huy học tập toàn diện.
  ...
  - **Phút Luyện Tập 7 Ngày Theo Từng Kỹ Năng (Per-Skill Analytics)**: Biểu đồ SVG đường mỏng 1.5px uốn mượt 7 ngày độc lập cho từng tab kỹ năng (**Dictation**, **Shadowing**, **Nói**, **Từ vựng**, **Viết**). Tự động thay đổi dữ liệu 7 ngày, tổng số phút, mã màu Gradient và SVG scaling khi chuyển tab. Đồng bộ dữ liệu qua `lib/store/skillChartStore.ts` và được bảo chứng bởi bộ 200+ kịch bản kiểm thử tự động (`scripts/run_skill_chart_200_tests.js`) đạt 100% Pass.
