...
export default function AiModuleError({ error, reset }: ErrorProps) {
  useEffect(() => {
    console.error("AI module routing error caught:", error);
  }, [error]);
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[70vh] px-4 py-12 text-center select-none animate-fade-in">
      <div className="w-full max-w-md p-6 sm:p-8 rounded-2xl bg-white/95 dark:bg-slate-900/95 backdrop-blur-2xl border border-slate-200/90 dark:border-slate-800 shadow-xl flex flex-col items-center space-y-5 relative overflow-hidden">
        ...
        <div className="absolute top-0 inset-x-0 h-1 bg-gradient-to-r from-transparent via-purple-500/60 to-transparent" />

        {/* Hero Icon Badge */}
        <div className="w-14 h-14 sm:w-16 sm:h-16 rounded-2xl bg-purple-50 dark:bg-purple-950/60 border border-purple-200/80 dark:border-purple-800/80 flex items-center justify-center text-purple-600 dark:text-purple-400 shadow-sm shrink-0">
        ...
        <div className="space-y-2 max-w-sm">
          <h2 className="text-xl sm:text-2xl font-black text-slate-900 dark:text-white font-display tracking-tight">
            Không khởi tạo được AI!
          </h2>
          <p className="text-xs sm:text-sm text-slate-600 dark:text-slate-300 leading-relaxed font-medium">
            Trợ lý AI đang bận hoặc gián đoạn kết nối server. Vui lòng làm mới trang hoặc thử lại.
          </p>

          {error.message && (
            <div className="text-xs font-mono p-3 rounded-xl bg-purple-50/60 dark:bg-purple-950/40 border border-purple-200/70 dark:border-purple-900/40 text-purple-700 dark:text-purple-300 max-h-24 overflow-y-auto break-all mt-3 text-left">
              {error.message}
            </div>
          )}
        </div>
