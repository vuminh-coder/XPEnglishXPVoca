...
export default function StudyPlanPage() {
  ...
  const [currentLevel] = useState<string>("Intermediate");
  ...
  if (isLoading) {
    return (
      <div className="flex h-[60vh] flex-col items-center justify-center space-y-4">
        ...
        <p className="text-sm font-semibold text-slate-500 dark:text-slate-400">Đang tải lộ trình học AI của bạn...</p>
      </div>
    );
  }
  ...
  if (!plan) {
    return (
      <div className="mx-auto max-w-3xl space-y-8" suppressHydrationWarning>
        <motion.div
        ...
        >
          <div className="inline-flex items-center gap-2 rounded-full border border-sky-200/50 bg-sky-50 dark:bg-sky-950/20 px-3.5 py-1.5 text-xs font-bold uppercase tracking-wider text-sky-650 dark:border-sky-900/30">
            <Sparkles className="h-4 w-4 text-sky-555 animate-pulse" />
          ...
          <form onSubmit={handleGenerate} className="bezel-inner bg-white dark:bg-neutral-900 p-6 md:p-8 space-y-6">
            <div className="grid gap-6 md:grid-cols-2">
              <div className="space-y-2.5">
                <label className="text-[10px] font-black uppercase tracking-wider text-slate-400">Kỳ thi mục tiêu</label>
                <div className="grid grid-cols-2 gap-3">
                  <button
                    ...
                    className={`bezel text-center transition-all ${targetExam === "TOEIC" ? "ring-2 ring-sky-400" : ""}`}
                  >
                    <div className="bezel-inner py-3 font-bold text-sm bg-white dark:bg-neutral-900 text-slate-700 dark:text-slate-350 cursor-pointer">TOEIC</div>
                  </button>
                  <button
                    type="button"
                    onClick={() => {
                    ...
                    className={`bezel text-center transition-all ${targetExam === "IELTS" ? "ring-2 ring-sky-400" : ""}`}
                  >
                    <div className="bezel-inner py-3 font-bold text-sm bg-white dark:bg-neutral-900 text-slate-700 dark:text-slate-350 cursor-pointer">IELTS</div>
                  </button>
                </div>
              </div>

              <div className="space-y-2.5">
                <label className="text-[10px] font-black uppercase tracking-wider text-slate-400">Điểm số mục tiêu</label>
                {targetExam === "TOEIC" ? (
                  <select
                    value={targetScore}
                    onChange={(e) => setTargetScore(e.target.value)}
                    className="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm font-semibold text-slate-800 focus:outline-none dark:border-neutral-850 dark:bg-neutral-950 dark:text-slate-200"
                  >
            ...
            <div className="grid gap-6 md:grid-cols-2">
              <div className="space-y-2.5">
                <label className="text-[10px] font-black uppercase tracking-wider text-slate-400">Ngày thi dự kiến</label>
              ...
              <div className="space-y-2.5">
                <label className="text-[10px] font-black uppercase tracking-wider text-slate-400">Cam kết học tập</label>
                <select
                  value={weeklyHours}
            ...
            <div className="pt-4">
              <Button type="submit" variant="primary" className="w-full py-4 text-xs md:text-sm font-bold flex items-center justify-center gap-2 cursor-pointer shadow-glow">
                Tạo Lộ trình học AI
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-8" suppressHydrationWarning>
      {/* Header Info Panel */}
      <motion.div
        initial={{ opacity: 0, y: -15 }}
        animate={{ opacity: 1, y: 0 }}
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div className="space-y-2">
              <span className="inline-flex items-center gap-1.5 rounded-full border border-white/25 bg-white/10 px-3 py-1 text-[10px] font-bold uppercase tracking-wider">
                <Trophy className="h-3.5 w-3.5 text-yellow-300 animate-bounce" />
                Mục tiêu: {plan.targetExam} · {plan.targetScore}
              </span>
              <h1 className="text-2xl md:text-3xl font-black tracking-tight font-display">Lộ trình học tập của bạn</h1>
              <p className="text-xs md:text-sm text-white/80 max-w-md font-semibold">
            ...
            <div className="min-w-[200px] space-y-2">
              ...
              <div className="text-[10px] text-white/70 text-right font-bold">
      ...
      <div className="grid gap-8 lg:grid-cols-5 items-start">
        ...
        <motion.div
          ...
          transition={{ type: "spring", stiffness: 85, damping: 15, delay: 0.1 }}
          className="lg:col-span-3 flex flex-col items-center py-6 bg-slate-50 dark:bg-neutral-950/40 rounded-[30px] border border-slate-100 dark:border-neutral-850 p-6 relative overflow-hidden"
        >
          <div className="absolute top-0 bottom-0 w-1.5 bg-slate-200 dark:bg-neutral-850 left-1/2 -translate-x-1/2 z-0" />
          ...
          <div className="space-y-12 w-full max-w-md relative z-10">
            {plan.dailyTasks.map((task, idx) => {
              ...
              let nodeBg = "bg-white border-slate-250 dark:bg-neutral-900 dark:border-neutral-850 text-slate-400";
              if (task.isCompleted) {
              ...
              return (
                <div key={task.id} className={`flex flex-col items-center transition-all ${translationClass}`}>
                  <motion.button
                    ...
                    className={`h-16 w-16 rounded-full border-4 flex items-center justify-center font-bold text-lg transition-transform cursor-pointer ${nodeBg} ${isSelected ? "ring-4 ring-indigo-400" : ""}`}
                  >
                    {isLocked ? (
                  ...
                  <span className="mt-2 text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-wider">
                    Ngày {idx + 1}
                  </span>
                </div>
              );
        ...
        <div className="lg:col-span-2 space-y-6 lg:sticky lg:top-24">
            ...
            {selectedTask ? (
              ...
              >
                <div className="bezel-inner bg-white dark:bg-neutral-900 p-6 space-y-6">
                  <div className="flex items-center justify-between border-b border-slate-100 dark:border-neutral-850 pb-4">
                    <div>
                      <span className="text-[10px] font-black uppercase tracking-wider text-slate-400">Chi tiết nhiệm vụ</span>
                      <h2 className="text-lg font-black text-slate-900 dark:text-white font-display">Ngày học tập</h2>
                    </div>
                    <Badge variant={selectedTask.isCompleted ? "success" : "primary"} className="font-bold">
                      {selectedTask.isCompleted ? "Đã xong" : "Chưa xong"}
                    </Badge>
                  </div>

                  <div className="flex items-start gap-4 bg-slate-50 dark:bg-neutral-950 p-4 rounded-2xl border border-slate-100 dark:border-neutral-850">
                    <div className="rounded-xl bg-sky-100 dark:bg-sky-950 p-2.5 text-sky-655 shrink-0">
                      {getTaskIcon(selectedTask.taskType)}
                    </div>
                    <div>
                      <h3 className="text-[10px] font-black uppercase tracking-wider text-slate-400">{selectedTask.taskType}</h3>
                      <p className="mt-1 text-xs text-slate-700 dark:text-slate-300 font-bold leading-relaxed">
                        {selectedTask.description}
                      </p>
                    </div>
                  </div>

                  <div className="flex items-center justify-between text-sm bg-indigo-50/50 dark:bg-indigo-950/20 p-4 rounded-2xl border border-indigo-100/50 dark:border-indigo-900/30">
                    <div className="flex items-center gap-2">
                      <Trophy className="h-5 w-5 text-amber-500 animate-bounce" />
                      <span className="font-black text-indigo-950 dark:text-indigo-400 text-xs uppercase">Phần thưởng XP</span>
                    </div>
                    <span className="font-black text-lg text-indigo-600 dark:text-indigo-455 font-display">+{selectedTask.xpReward} XP</span>
                  </div>

                  <div className="flex flex-col gap-3 pt-2">
                    <Button
                      variant={selectedTask.isCompleted ? "bezel" : "primary"}
                      className="w-full py-3.5 text-xs font-bold cursor-pointer rounded-xl"
                    ...
                    {!selectedTask.isCompleted && (
                      <Link href="/study/practice" className="w-full">
                        <Button variant="bezel" className="w-full py-3.5 text-xs font-bold flex items-center justify-center gap-1.5 cursor-pointer rounded-xl">
              ...
              >
                <div className="bezel-inner bg-white dark:bg-neutral-900 p-8 text-center space-y-4">
                  <div className="mx-auto flex h-16 w-16 items-center justify-center rounded-full bg-slate-50 dark:bg-neutral-950 border border-slate-100 dark:border-neutral-850">
                    <Compass className="h-8 w-8 text-slate-400" />
                  </div>
                  <h3 className="text-base font-black text-slate-900 dark:text-white font-display">Khám phá hành trình</h3>
          ...
          <div className="bezel">
            <div className="bezel-inner bg-slate-50 dark:bg-neutral-950 p-5 flex items-center justify-between">
              <div>
                <h4 className="text-[10px] font-black uppercase tracking-wider text-slate-400">Thay đổi mục tiêu?</h4>
                <p className="text-[10px] text-slate-500 mt-0.5 font-semibold">Tạo lại lộ trình học mới bất kỳ lúc nào.</p>
              </div>
              <Button
                variant="ghost"
                size="sm"
