...
export default function CommunityError({ error, reset }: ErrorProps) {
  useEffect(() => {
    console.error("Community module routing error caught:", error);
  }, [error]);
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[70vh] px-4 py-12 text-center select-none animate-fade-in">
      <div className="w-full max-w-md p-6 sm:p-8 rounded-2xl bg-white/95 dark:bg-slate-900/95 backdrop-blur-2xl border border-slate-200/90 dark:border-slate-800 shadow-xl flex flex-col items-center space-y-5 relative overflow-hidden">
        ...
        {/* Hero Icon Badge */}
        <div className="w-14 h-14 sm:w-16 sm:h-16 rounded-2xl bg-amber-50 dark:bg-amber-950/60 border border-amber-200/80 dark:border-amber-800/80 flex items-center justify-center text-amber-600 dark:text-amber-400 shadow-sm shrink-0">
        ...
        <div className="space-y-2 max-w-sm">
          ...
          <p className="text-xs sm:text-sm text-slate-600 dark:text-slate-300 leading-relaxed font-medium">
            Không thể tải bảng tin hoặc kết nối mạng bị gián đoạn. Vui lòng thử lại.
          </p>

          {error.message && (
            <div className="text-xs font-mono p-3 rounded-xl bg-amber-50/60 dark:bg-amber-950/40 border border-amber-200/70 dark:border-amber-900/40 text-amber-800 dark:text-amber-300 max-h-24 overflow-y-auto break-all mt-3 text-left">
              {error.message}
            </div>
          )}
        </div>
