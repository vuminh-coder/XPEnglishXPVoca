...
export default function GroupsPage() {
  ...
  return (
    <div className="space-y-3.5 sm:space-y-6 pb-16 md:pb-6 select-none font-sans" suppressHydrationWarning>
      ...
      <div className="p-3.5 sm:p-6 rounded-md sm:rounded-lg bg-gradient-to-r from-[#0059bb] via-[#004799] to-[#312e81] text-white shadow-xs relative overflow-hidden">
        ...
        <div className="relative z-10 space-y-2 sm:space-y-3">
          <div className="flex items-center gap-1.5 sm:gap-2 flex-nowrap whitespace-nowrap overflow-x-auto no-scrollbar">
            <span className="px-1.5 py-0.5 sm:px-2.5 sm:py-0.5 rounded-sm text-[9px] sm:text-[11px] font-black uppercase tracking-wider bg-white/15 text-white border border-white/20 flex items-center gap-1 font-display shrink-0">
              <Users className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-indigo-300" /> Nhóm Học Tập
            </span>
            <span className="px-1.5 py-0.5 sm:px-2.5 sm:py-0.5 rounded-sm text-[9px] sm:text-[11px] font-bold bg-indigo-500/20 text-indigo-200 border border-indigo-400/30 shrink-0">
              Cấp 15+ Để Tạo Nhóm
            </span>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-3.5 sm:gap-5 items-start">
        ...
        <div className="lg:col-span-8 xl:col-span-8 space-y-3.5 sm:space-y-5">
          ...
          <div className="p-3 sm:p-5 rounded-md sm:rounded-lg bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-white/10 shadow-xs flex flex-row items-center justify-between gap-3">
            ...
            <Button
              ...
              className="px-2.5 sm:px-4 py-1.5 sm:py-2 rounded-sm sm:rounded-md bg-[#0059bb] hover:bg-[#004799] text-white text-xs font-bold transition-all shadow-2xs shrink-0 cursor-pointer flex items-center gap-1 sm:gap-1.5"
            >
          ...
          <div className="space-y-3 sm:space-y-4">
            <h3 className="text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400 flex items-center gap-1.5 font-display">
              <Globe className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-[#0059bb] dark:text-sky-400" /> Danh Sách Nhóm Học Tập Cộng Đồng:
            </h3>
            ...
            {loading ? (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                {[1, 2, 3, 4].map((i) => (
                  <div key={i} className="p-3.5 sm:p-4 rounded-md sm:rounded-lg bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-white/10 shadow-xs space-y-3 animate-pulse">
                    <div className="flex items-center gap-3">
                      <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-sm sm:rounded-md bg-slate-200 dark:bg-slate-800" />
              ...
              <div className="p-6 sm:p-8 rounded-md sm:rounded-lg bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-white/10 text-center space-y-2">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                {groups.map((g) => (
                  <div
                    key={g.id}
                    className="p-3.5 sm:p-4 rounded-md sm:rounded-lg bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-white/10 shadow-xs flex flex-col justify-between space-y-3 sm:space-y-3.5 hover:border-[#0059bb]/40 transition-all"
                  >
                    <div className="space-y-2 sm:space-y-2.5">
                      <div className="flex items-center justify-between gap-2">
                        <div className="flex items-center gap-2 sm:gap-2.5 min-w-0">
                          <div className="w-8 h-8 sm:w-9 sm:h-9 rounded-sm sm:rounded-md bg-[#0059bb]/10 text-[#0059bb] dark:text-sky-400 flex items-center justify-center shrink-0 border border-[#0059bb]/20">
                            {GROUP_ICONS[g.id] || <Users className="w-4 h-4 sm:w-5 sm:h-5 text-[#0059bb]" />}
                          </div>
                        ...
                        <span className="text-[9px] sm:text-[10px] font-bold px-1.5 py-0.5 rounded-sm bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 border border-slate-200/60 font-mono shrink-0">
                          {g.memberCount || 1} TV
                        </span>
                      </div>
                    ...
                    <button
                      ...
                      className={`w-full py-1.5 sm:py-2 px-3 rounded-sm sm:rounded-md text-xs font-bold transition-all shadow-2xs cursor-pointer flex items-center justify-center gap-1.5 ${
                        g.joined
        ...
        <div className="lg:col-span-4 xl:col-span-4 space-y-3.5 sm:space-y-4 sticky top-4">
            ...
            {myJoinedGroups.length === 0 ? (
              ...
              <div className="space-y-1.5 sm:space-y-2">
                {myJoinedGroups.map((mg) => (
                  <div
                    key={mg.id}
                    className="p-2 sm:p-2.5 rounded-sm bg-slate-50 dark:bg-slate-800/60 border border-slate-200/60 dark:border-white/5 flex items-center justify-between text-xs"
                  >
                    <span className="font-bold text-slate-900 dark:text-white truncate font-display">
                      {mg.name}
                    </span>
                    <span className="text-[9px] sm:text-[10px] font-bold text-emerald-600 bg-emerald-500/10 px-1.5 py-0.2 rounded-sm shrink-0">
                      Đã tham gia
                    </span>
                  </div>
                ))}
          ...
          <div className="p-3.5 sm:p-4.5 rounded-md sm:rounded-lg bg-[#ebf3fe]/80 dark:bg-slate-800/60 border border-[#0059bb]/20 shadow-xs space-y-1.5 sm:space-y-2">
            <div className="flex items-center gap-1.5 text-xs font-bold text-[#0059bb] dark:text-sky-400 font-display">
              <ShieldCheck className="w-3.5 h-3.5 sm:w-4 sm:h-4" /> Đạt Cấp 15 Để Tạo Nhóm Riêng
            </div>
