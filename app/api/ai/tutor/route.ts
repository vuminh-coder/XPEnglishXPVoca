...
const TUTOR_TOPIC_BANKS: Record<
  string,
  {
    reply: string;
...
> = {
  movies: {
    reply:
      "Movies are a fantastic way to learn English! What genre do you enjoy the most — action, sci-fi, or comedy?",
    vietnameseTranslation:
      "Phim ảnh là cách tuyệt vời để học tiếng Anh! Bạn thích thể loại nào nhất — hành động, viễn tưởng hay hài kịch?",
    words: [
  ...
  food: {
    reply:
      "Talking about food always makes me hungry! Do you prefer cooking at home or dining out with friends?",
    vietnameseTranslation:
      "Nói về đồ ăn luôn làm tôi thấy thèm! Bạn thích tự nấu ở nhà hay đi ăn ngoài với bạn bè hơn?",
    words: [
  ...
  travel: {
    reply:
      "Traveling is the best way to broaden your horizons! Have you visited any memorable places recently?",
    vietnameseTranslation:
      "Du lịch là cách tuyệt vời nhất để mở rộng tầm nhìn! Gần đây bạn có ghé thăm nơi nào đáng nhớ không?",
    words: [
  ...
  tech: {
    reply:
      "Technology is evolving so rapidly! What is your favorite tech gadget or AI app you use daily?",
    vietnameseTranslation:
      "Công nghệ đang phát triển rất nhanh! Thiết bị công nghệ hoặc ứng dụng AI bạn dùng hàng ngày là gì?",
    words: [
  ...
  work: {
    reply:
      "Balancing work and personal life is essential. What does a typical productive day look like for you?",
    vietnameseTranslation:
      "Cân bằng công việc và cuộc sống rất quan trọng. Một ngày làm việc hiệu quả của bạn diễn ra thế nào?",
    words: [
  ...
  general: {
    reply:
      "That is a very interesting thought! Could you elaborate a bit more on why you think so?",
    vietnameseTranslation:
      "Đó là một ý nghĩ rất thú vị! Bạn có thể chia sẻ thêm một chút về lý do bạn nghĩ như vậy không?",
    words: [
...
export async function POST(request: Request) {
  try {
    ...
    if (isRateLimited(`ai_tutor_${userId}`, 30, 60 * 1000)) {
    ...
    const { messages, message, history, persona = "emma", speed = 1.0 } = body;

    // Polymorphic payload support: handles either { messages } or { message, history }
    ...
    if (Array.isArray(messages) && messages.length > 0) {
      ...
      if (message && typeof message === "string" && message.trim()) {
        conversationMessages.push({ role: "user", text: message.trim() });
      }
    }

    if (conversationMessages.length === 0) {
      return NextResponse.json(
        { error: "Vui lòng cung cấp nội dung câu nói hoặc đoạn hội thoại." },
        { status: 400 }
      );
    }
...
3. Return ONLY raw JSON format. No markdown fences. Ensure valid JSON parsing.`;
    ...
    let contents = conversationMessages.map((m) => ({
    ...
    if (
      parsedData.grammarCorrection &&
      typeof parsedData.grammarCorrection.explanation === "string"
    ) {
    ...
    if (
      parsedData.betterPhrasing &&
      typeof parsedData.betterPhrasing === "string"
    ) {
      parsedData.betterPhrasing = parsedData.betterPhrasing
        .replace(/^["']|["']$/g, "")
        .replace(
          /^(A more natural way to say that (would be|is)|You can say|You could say|A better phrasing is|Try saying),?\s*/i,
          ""
        )
        .replace(/^["']|["']$/g, "")
        .trim();
    ...
    return NextResponse.json({
      success: true,
      reply:
        "Hello! I am your AI Voice Tutor. Please speak or type to begin our conversation.",
      vietnameseTranslation:
        "Xin chào! Tôi là Gia sư Giọng nói AI. Hãy nói hoặc nhập tin nhắn để bắt đầu cuộc trò chuyện nhé.",
